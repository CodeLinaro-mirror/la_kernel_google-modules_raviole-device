				...
				struct exynos_bcm_dbg_data *data)
{
	...
	ret = of_property_read_u32(np, "bcm_ip_print_nr",
				   &data->bcm_ip_print_nr);
	if (ret) {
		BCM_ERR("%s: Failed get bcm_ip_print_nr\n", __func__);
		return ret;
	}

	size = of_property_count_strings(np, "ip-name");
	if (size < 0) {
		BCM_ERR("%s: Failed get number of ip-name\n", __func__);
		return size;
	}

	if (size != data->bcm_ip_nr) {
		BCM_ERR("%s: ip-name (%d) is not same as bcm_ip_nr (%d)\n",
			__func__, size, data->bcm_ip_nr);
		return size;
	}

	data->bcm_ip_names = kzalloc(size * sizeof(char *), GFP_KERNEL);
	if (data->bcm_ip_names == NULL) {
		BCM_ERR("%s: failed to allocate BCM IP names\n", __func__);
		return -ENOMEM;
	}

	size = of_property_read_string_array(np, "ip-name",
					     data->bcm_ip_names, size);
	if (size < 0) {
		BCM_ERR("%s: Failed get ip-name\n", __func__);
		kfree(data->bcm_ip_names);
		return size;
	}
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	const unsigned int *event_data;

	ret = of_property_read_u32(np, "max_define_event",
				   &data->define_event_max);
	if (ret) {
	...
	for (i = 0; i < nr_event; i++) {
		event_data = &event_addr[i * event_len];
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	const unsigned int *id_data;
	const unsigned int *active_data;
	...
	filter_id_addr = of_get_property(np, "define_filter_id",
					 &filter_id_get_len);
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		id_data = &filter_id_addr[i * filter_id_len];
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		active_data = &filter_id_active_addr[i * active_len];
		...
		for (active_cnt = 0; active_cnt < BCM_EVT_EVENT_MAX;
		     active_cnt++)
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	unsigned int filter_other_0_get_len, filter_other_1_get_len;
	unsigned int active_get_len;
	const unsigned int *other0_data;
	const unsigned int *other1_data;
	const unsigned int *active_data;
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		other0_data = &filter_other_0_addr[i * filter_other_0_len];
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		other1_data = &filter_other_1_addr[i * filter_other_1_len];
	...
	filter_other_active_addr = of_get_property(np,
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		active_data = &filter_other_active_addr[i * active_len];
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	sample_id_addr = of_get_property(np, "define_sample_id",
					 &sample_id_get_len);
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		id_data = &sample_id_addr[i * sample_id_len];
	...
	for (i = 0; i < PRE_DEFINE_EVT_MAX; i++) {
		active_data = &sample_active_addr[i * active_len];
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	const unsigned int *stop_owner_data;


	ret = of_property_read_u32(np, "initial_bcm_run",
				   &data->initial_bcm_run);
	if (ret) {
	...
	ret = of_property_read_u32(np, "initial_bcm_mode",
				   &data->initial_bcm_mode);
	if (ret) {
	...
	stop_owner_addr = of_get_property(np, "available_stop_owner",
					  &stop_owner_get_len);
	...
	stop_owner_data = stop_owner_addr;
	for (i = 0; i < size; i++) {
