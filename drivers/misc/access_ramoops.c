...
struct access_ramoops_info {
	...
	void *addr;
	size_t size;
	struct mutex lock; /* protect access to structure */
	int is_mapped;
	int is_open;
...
static ssize_t access_ramoops_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
	void *kbuf;
	ssize_t ret;
	...
	ret = simple_read_from_buffer(buf, count, ppos, kbuf, info->size);
	vfree(kbuf);
	return ret;
}

static ssize_t access_ramoops_write(struct file *filp, const char __user *buf, size_t count,
				    loff_t *ppos)
{
	void *kbuf;
	ssize_t ret;
	...
	ret = simple_write_to_buffer(kbuf, info->size, ppos, buf, count);
	if (ret > 0)
